...
impl NodeExt for Section {
    fn into_node(self) -> Node {
        let mut output = Div.class("blog-section");
        ...
        for paragraph in self.paragraphs {
            ...
            for tidbit in paragraph.tidbits {
                let to_add: Node = match tidbit {
                    ...
                    Tidbit::Code(code) => Code.class("blog-code rounded").text(code).into_node(),
                    Tidbit::Shell(command) => {
                        Code.class("blog-shell rounded").text(command).into_node()
                    }
                    Tidbit::Text(text) => {
...
impl NodeExt for Article {
    fn into_node(self) -> Node {
        let mut output = Div.class("blog-article");
