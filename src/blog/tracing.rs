...
pub fn intro() -> Article {
    Article::new()
        ...
        .list(vec![
            ...
            "What is an event?",
            "What is a span?",
            "How can the act of entering a span impact code in the same scope? In other words, how can one part of code introduce spooky state on following code?",
            "Why do we have to be careful with spans when using async?",
            "What does the instrument attribute do, and what does the instrument call on a future do?",
            "How is global state managed? Specifically, how does logging interact with a global subscriber?",
        ])
        ...
        .p(" is a good way to go about things. There seems to be some nice integrations between tracing and OpenTelemetry. But OpenTelemetry too has concepts which I know little of. So...")
        .list(vec![
            "How does OpenTelemetry work?",
            "How well does it map to tracing?",
            "Which collectors should I use?",
            "Is a collector what I think it is: The thing which receives traces? Is it very close to being a (tracing) subscriber?",
        ])
}
